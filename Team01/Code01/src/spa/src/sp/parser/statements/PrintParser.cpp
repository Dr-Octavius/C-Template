...
std::shared_ptr<PrintNode> PrintParser::parse(ParserContext& ctx) {

    int lineNum = ctx.getCurrentLine();

    // Always begin each parse method with the assertion of the return type
    static_assert(std::is_base_of<ASTNode, PrintNode>::value, "<T> returned in ProcedureParser must be derived from ASTNode");

    // The next token should be an identifier (the procedure name)
    if (!ctx.expectToken(TokenType::KEYWORD, "print")) {
        throw SimpleSyntaxError("Expected print name token");
    }
    ...
    return std::make_shared<PrintNode>(varNode, lineNum);
}